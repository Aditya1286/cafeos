export const STATUS_CONFIG: Record<string, { label: string; color: string; borderAccent: string; badgeBg: string; badgeText: string; dot: string }> = {
...
export const isOrderCancellable = (status: string) => !['CANCELLED', 'COMPLETED', 'REFUNDED'].includes(status);
