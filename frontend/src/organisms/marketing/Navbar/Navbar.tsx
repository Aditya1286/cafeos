    ...
    <nav className="sticky top-0 z-40 w-full bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 lg:px-8 py-3.5 shadow-sm">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2.5 group">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-semibold">
              Ordering for Cafés & Restaurants
            </p>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-6 text-sm font-semibold text-slate-700">
          ...
          <Link to="/pricing" className="hover:text-red-600 transition-colors">
            Pricing
          </Link>
          <Link
            to="/c/artisan-cafe/t/tok_artisan_tbl_01"
            className="flex items-center gap-1.5 text-xs bg-slate-100 hover:bg-slate-200/80 px-3 py-1.5 rounded-lg border border-slate-200 text-slate-800 transition-all"
          >
          ...
          {token ? (
            <div className="flex items-center gap-3">
              {user?.role === 'SUPER_ADMIN' ? (
                <Link
                  to="/admin"
                  className="flex items-center gap-1.5 text-xs font-bold px-3.5 py-2 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition-all"
                >
                ...
                <Link
                  to="/dashboard"
                  className="flex items-center gap-1.5 text-xs font-bold px-3.5 py-2 rounded-xl bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 transition-all"
                >
            ...
            <div className="flex items-center gap-2">
              <Link
                to="/login"
                className="px-4 py-2 text-xs font-bold text-slate-700 hover:text-red-600 transition-colors"
              >
                Sign In
              </Link>
              <Link
                to="/register"
                className="px-5 py-2 text-xs font-bold rounded-xl bg-red-600 hover:bg-red-700 text-white shadow-md shadow-red-600/20 transition-all"
              >
                Start Free Trial
              </Link>
            </div>
