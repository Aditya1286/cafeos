...
import { MasterQrCard } from './MasterQrCard';
...
interface TablesQrPanelProps {
  ...
  /** The general menu link (/c/:slug) the master QR encodes. */
  menuUrl: string;
  masterQrEnabled: boolean;
  savingMasterQr: boolean;
  onToggleMasterQr: () => void;
  onAddTable: () => void;
...
export const TablesQrPanel = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {!tablesEnabled ? (
        <div className="bg-white rounded-3xl border border-dashed border-slate-300 p-12 text-center space-y-2">
          ...
          <h3 className="text-sm font-black text-slate-700">
            Tables are disabled for this business
          </h3>
          ...
          <div className="space-y-5 text-center">
            ...
            <div className="space-y-1">
              <p className="text-sm font-bold text-slate-800">
                Delete <span className="font-mono text-rose-600">{tableToDelete.tableNumber}</span>{' '}
                and its QR code?
              </p>
              <p className="text-xs text-slate-500 font-medium">
                This can't be undone — any printed QR for this table will stop working. Tables with
                an order still in progress can't be deleted.
              </p>
            </div>
      ...
      {tableToEmpty && (
        <Modal
          title="Mark this table as empty?"
          onClose={() => setTableToEmpty(null)}
          maxWidth="max-w-sm"
        >
          <div className="space-y-5 text-center">
            ...
            <div className="space-y-1">
              <p className="text-sm font-bold text-slate-800">
                Clear the "Occupied" status on{' '}
                <span className="font-mono text-amber-700">{tableToEmpty.tableNumber}</span>?
              </p>
              <p className="text-xs text-slate-500 font-medium">
                Use this if the table is showing occupied by mistake. This only clears the indicator
                — it won't cancel or change any order still linked to this table.
              </p>
            </div>
            <div className="flex gap-3">
              <button
