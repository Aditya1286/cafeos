    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 sm:gap-6">
        {cards.map((card, i) => (
          <div
            key={card.label}
            className={`p-4 sm:p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-1 sm:space-y-2 min-w-0 ${i === cards.length - 1 ? 'col-span-2 md:col-span-1' : ''}`}
          >
            <div className="text-[10px] sm:text-xs text-slate-400 font-bold uppercase">
              {card.label}
            </div>
            <div className={`text-xl sm:text-3xl font-black truncate ${card.valueColor}`}>
              {card.value}
            </div>
      ...
      <div className="bg-white p-4 sm:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <h3 className="text-base sm:text-lg font-extrabold text-slate-900">How Customers Paid</h3>
          <button
            onClick={onExportCsv}
              ...
              {paymentMethodBreakdown.map((pm) => {
                const pct =
                  totalBreakdownAmount > 0
                    ? Math.round((pm.amountPaise / totalBreakdownAmount) * 100)
                    : 0;
                return (
                  <div
                    key={pm._id}
                  ...
                  >
                    <div>
                      <div className="text-xs text-slate-500 font-bold">
                        {pm._id} ({pm.count} orders)
                      </div>
                      <div className="text-lg font-extrabold text-slate-900">
                        {formatCurrency(pm.amountPaise)}
                      </div>
                    </div>
