...
export const BusinessAvatar = ({ business, size = 'md' }: { business: AdminBusinessSummary; size?: 'sm' | 'md' }) => {
  const health = businessHealthOf(business);
  return (
    <div className={`relative shrink-0 rounded-xl border flex items-center justify-center font-black ${AVATAR_STYLE[health]} ${
      size === 'sm' ? 'w-9 h-9 text-xs' : 'w-11 h-11 text-sm'
    }`}>
      {business.name.charAt(0).toUpperCase()}
      {health !== 'healthy' && (
        <span className={`absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full border-2 border-white ${DOT_STYLE[health]}`} />
      )}
    </div>
  );
};
...
export const BusinessBadges = ({ business }: { business: AdminBusinessSummary }) => (
  <div className="flex flex-wrap items-center gap-1">
    {business.status === 'SUSPENDED' && (
      <span className="px-2 py-0.5 rounded-full bg-rose-100 text-rose-800 border border-rose-200 text-[10px] font-extrabold">SUSPENDED</span>
    )}
    {business.overdueAmountPaise > 0 && (
      <span className="px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-300 text-[10px] font-extrabold">OVERDUE</span>
    )}
...
export const BusinessContactLinks = ({ business }: { business: AdminBusinessSummary }) => {
  const linkCls = 'flex-1 min-w-0 flex items-center justify-center gap-1.5 px-2.5 py-2 rounded-xl bg-white border border-slate-200 text-[11px] font-bold text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-colors';
  return (
    <div className="flex items-center gap-2">
      {business.phone && (
        <a href={`tel:${business.phone.replace(/\s+/g, '')}`} className={linkCls} title={business.phone}>
          <Phone className="w-3.5 h-3.5 text-emerald-600 shrink-0" /> <span className="truncate">Call</span>
        </a>
      )}
      {business.email && (
        <a href={`mailto:${business.email}`} className={linkCls} title={business.email}>
          <Mail className="w-3.5 h-3.5 text-blue-600 shrink-0" /> <span className="truncate">Email</span>
        </a>
      )}
      <a href={`/c/${business.slug}`} target="_blank" rel="noopener noreferrer" className={linkCls} title={`/c/${business.slug}`}>
        <ExternalLink className="w-3.5 h-3.5 text-red-600 shrink-0" /> <span className="truncate">Menu</span>
      </a>
    </div>
  );
};

export const DemoToggle = ({ business, updatingDemoId, onSetDemo }: { business: AdminBusinessSummary } & Pick<BusinessRowActions, 'updatingDemoId' | 'onSetDemo'>) => {
  const saving = updatingDemoId === business._id;
  return (
    <label className={`flex items-center justify-between gap-3 px-3 py-2 rounded-xl border transition-colors ${
      ...
      <span className="min-w-0">
        ...
        <span className="block text-[10px] font-medium text-slate-500">Not counted in platform reports</span>
      </span>
      <input
        type="checkbox"
...
export const BusinessActionButtons = ({ business, onOpenFinance, onOpenStatusModal }: { business: AdminBusinessSummary } & Pick<BusinessRowActions, 'onOpenFinance' | 'onOpenStatusModal'>) => (
  <div className="flex items-center gap-2">
    <button
    ...
    <button
      onClick={() => onOpenStatusModal({ id: business._id, name: business.name, status: business.status })}
    ...
    >
      {business.status === 'ACTIVE' ? <><Ban className="w-3.5 h-3.5" /> Suspend</> : <><RotateCcw className="w-3.5 h-3.5" /> Reactivate</>}
    </button>
  </div>
);
