...
export const KpiOverviewBentoMobile = ({ tiles }: { tiles: KpiTile[] }) => (
  <div className="grid grid-cols-2 gap-2.5">
    {tiles.map((t) => (
      <div key={t.key} className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-2 min-w-0">
        <div className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center [&>svg]:w-4 [&>svg]:h-4">{t.icon}</div>
