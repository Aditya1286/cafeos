...
const InsightCard = ({
  ...
  <div className="bg-white p-3 sm:p-4 rounded-2xl border border-slate-200 shadow-xs space-y-2 min-w-0">
    <div className="flex items-start justify-between gap-2">
      <span className="text-[10px] font-extrabold uppercase text-slate-400 tracking-wider leading-tight">
        {label}
      </span>
      <div
      ...
      ? [
          {
            ...
            render: (o: (typeof rows)[number]) => (
              <div className="font-black text-slate-900 truncate max-w-[160px]">
                {o._businessName || '—'}
              </div>
            ),
    ...
    },
    {
      ...
      render: (o) => (
        <button
          ...
          className="px-3 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-black transition-colors flex items-center gap-1 ml-auto"
        >
  ...
  const renderCard = (o: (typeof rows)[number]) => (
    <div className="p-4 space-y-3">
      ...
      <div className="text-[11px] text-slate-400 font-medium">
        {o._dateFormatted}
        {o._businessName ? ` · ${o._businessName}` : ''}
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 bg-white p-4 sm:p-5 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Platform-wide, or narrowed to one business — what got cancelled, what's still owed, and
            ...
            <div className="absolute z-20 top-full mt-1.5 left-0 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
              {searchResults.length === 0 ? (
                <div className="px-3.5 py-3 text-xs text-slate-400 font-medium">
                  No business matches "{businessQuery}"
                </div>
              ) : (
                searchResults.map((b) => (
                  <button
        ...
        {selectedBusiness ? (
          <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-red-50 border border-red-200 text-xs font-bold text-red-700 min-w-0 sm:max-w-xs">
            ...
            <button
              ...
              className="p-0.5 rounded-md hover:bg-red-100 shrink-0"
            >
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-slate-100">
                <h3 className="text-xs font-extrabold text-slate-700 uppercase tracking-wider">
                  Top Businesses by Refund Volume
                </h3>
              </div>
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-xs font-black text-violet-600 shrink-0">
                        {formatCurrency(b.refundedAmountPaise)}
                      </span>
                    </div>
                    <div className="text-[11px] font-semibold text-slate-500">
                      {b.cancellations} cancelled · {b.refundedCount} refunded
                      {b.needsRefundCount > 0 && (
                        <span className="text-amber-600 font-bold">
                          {' '}
                          · {b.needsRefundCount} need refund
                        </span>
                      )}
                    </div>
                  </button>
                    ...
                    {insights.byBusiness.map((b) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 text-right font-bold">
                          <span
                            className={b.needsRefundCount > 0 ? 'text-amber-600' : 'text-slate-400'}
                          >
                            {b.needsRefundCount}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-2.5 text-right font-black text-violet-600">
                          {formatCurrency(b.refundedAmountPaise)}
                        </td>
                      </tr>
                    ))}
