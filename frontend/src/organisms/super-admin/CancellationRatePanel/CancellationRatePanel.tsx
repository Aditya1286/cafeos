 ...
 */
export const CancellationRatePanel = ({ stats, timeseries, worstBusinesses }: CancellationRatePanelProps) => {
  ...
  return (
    <div className="lg:col-span-6 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-5">
      ...
      {!stats || stats.rate === null ? (
        <p className="text-xs text-slate-400 font-medium py-6 text-center">No completed, cancelled, or refunded orders yet in this range.</p>
      ) : (
        <>
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <p className="text-[11px] font-semibold text-slate-500 mt-0.5">
                {stats.cancelledCount} cancelled · {stats.refundedCount} refunded of {stats.totalTerminalCount} orders
              </p>
            </div>
          </div>
            ...
            <div className="h-32">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <YAxis stroke="#94a3b8" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(v) => `${v}%`} width={32} />
                  <Tooltip
                    contentStyle={adminTooltipStyle}
                    formatter={(value: number, _name, item: any) => [item?.payload?.hasData ? `${value}%` : 'No orders', 'Cancel/refund rate']}
                  />
              ...
              <div className="space-y-2">
                {worstBusinesses.map((b) => (
                  <div key={b.businessId} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                    ...
                    <div className={`text-sm font-black shrink-0 flex items-center gap-1 ${rateColor(b.rate)}`}>
