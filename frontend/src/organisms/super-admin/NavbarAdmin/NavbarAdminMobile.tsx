...
export const NavbarAdminMobile = ({
  activeTab, onTabChange, onOpenCommand, dateRange, onDateRangeChange, onExport, user, onLogout, navItems,
  ...
  const hasPendingWork = navItems.some((item) => ACTIONABLE_NAV_IDS.has(item.id) && (item.count || 0) > 0);
  ...
  const runAndClose = (fn: () => void) => () => { closeDrawer(); fn(); };
          ...
          <>
            ...
            >
              <div className="h-14 px-3 flex items-center justify-between border-b border-slate-100 shrink-0">
                <span className="px-2 text-[11px] font-extrabold uppercase tracking-wider text-slate-400">Menu</span>
                <button
                  onClick={closeDrawer}
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-3">
                ...
                <div className="space-y-0.5">
                  {navItems.map((item) => {
                    ...
                    return (
                      <button
                        key={item.id}
                        onClick={() => { onTabChange(item.id); closeDrawer(); }}
                        aria-current={active ? 'page' : undefined}
                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${
                          active ? 'bg-red-50 text-red-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
                        }`}
                      >
                        <Icon className={`w-4 h-4 shrink-0 ${active ? 'text-red-600' : 'text-slate-400'}`} />
                        <span className="flex-1 min-w-0 text-xs font-bold truncate">{item.label}</span>
                        {item.count !== undefined && (
                          <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-mono shrink-0 ${
                            actionable ? 'bg-red-600 text-white' : 'bg-slate-100 text-slate-500'
                          }`}>
                            {item.count}
                          </span>
                        )}
                ...
                <div className="pt-2 border-t border-slate-100 space-y-0.5">
                  <span className="block px-3 pb-1 text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Export</span>
                  <button
              ...
              <div className="p-3 border-t border-slate-100 shrink-0">
                <div className="flex items-center gap-2.5 px-2 py-1.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-extrabold text-slate-900 truncate">{user?.name || 'Super Admin'}</div>
